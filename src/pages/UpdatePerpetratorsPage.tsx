...
import {
  getPerpetratorDetailsById,
  updatePerpetrator,
} from "../service/perpetratorService";
...
const UpdatePerpetratorsPage = () => {
  const history = useNavigate();
  const location = useLocation();
  const dispatch = useDispatch();

  const [perpetratorId, setPerpetratorId] = useState<number>();
  ...
  useEffect(() => {
    ...
    if (state && state.perpetratorId) {
      ...
      console.log(perpetratorId, "perpetratorId");
  ...
  const getPerpetratorDetails = (perpetratorId: number) => {
    ...
    getPerpetratorDetailsById(perpetratorId)
      .then((response) => {
        //get details and set to inputs
        //setDetailsToInputs(response)
  ...
  const setDetailsToInputs = (details) => {
    setPerpetratorId(undefined);
    setFullName("");
    setContactNo("");
    setAge("");
    setNICNumber("");
    setDescription("");
    setPastCriminalRecords("");
    setGender("");
    setPerpetratorProfileImgs([]);
    setFileList([]);
    setDNASequence("");
    ...
    // subProductData.files?.map(async (f, index) => {

    //     temp.push(f);

    // });
    // setCurrentImageFileList(temp);

    // form.setFieldsValue({
    //   fullName: selectedRoomTypeDetails,
    //   contactNo: guestCountOfRoomDetails,
    //   age: roomNameDetails,
    //   NICNumber selectedRoomTypeDetails,
    //   description: guestCountOfRoomDetails,
    //   pastCriminalRecords: roomNameDetails,
    //   gender: roomNameDetails,
    //   perpetratorProfileImgs: roomNameDetails,
    //   DNASequence: roomNameDetails,
    // });
  };
  ...
  const customRequest = async (file: any, onSuccess: any, onError: any) => {
    ...
    try {
      ...
      setPerpetratorProfileImgs(temp);
      setIsUploading(true);
      onSuccess();
    } catch (error: any) {
  ...
  const handleUpdatePerpetrator = () => {
    ...
    if (isValidate) {
      ...
      const payload = {
        ...
        gender: gender,
        files: perpetratorProfileImgs,
        dna: DNASequence,
      };

      updatePerpetrator(perpetratorId, payload)
        .then((response: any) => {
          popUploader(dispatch, false);
          customToastMsg("Perpetrator profile updated successfully", 1);
          ...
          <div className="mainTextDiv w-90">
            ...
            >
              <Row className="w-100">
                ...
                <Col xs={24} sm={24} md={12} className="ps-0 ps-md-3">
                  <Form.Item
                    className="align-self-start w-100 ps-5 ps-lg-0 mt-5"
            ...
            <Row className="my-5 pb-5 d-flex justify-content-end">
              <Button
                ...
                size="large"
                type="default"
                onClick={handleUpdatePerpetrator}
              >
                Update Perpetrator
              </Button>
            </Row>
...
export default UpdatePerpetratorsPage;
